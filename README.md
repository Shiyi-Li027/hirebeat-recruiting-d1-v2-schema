...
- Durable `catalog_sync_run` / `catalog_sync_target_run` result-reporting code is implemented and locally validated. Before enabling a production provider-native submission window, deploy and verify it in staging, add automatic retry dispatch before relying on `failed_retryable` recovery, and configure production-only provider credentials and identifiers. Staging resources and Secrets must not be reused.
