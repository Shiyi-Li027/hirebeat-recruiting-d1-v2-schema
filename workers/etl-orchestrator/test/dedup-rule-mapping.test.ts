...
import { evidenceTypeForFeature, primaryMatchRule } from "../src/workflow-a-dedup";
...
test("logical identity features map to the reviewed evidence codes", () => {
  assert.equal(evidenceTypeForFeature("email"),"email_exact_match");
  assert.equal(evidenceTypeForFeature("phone"),"phone_last_10_exact_match");
  assert.equal(evidenceTypeForFeature("linkedin_url"),"linkedin_exact_match");
  assert.equal(evidenceTypeForFeature("github_url"),"github_exact_match_with_same_normalized_last_name");
  assert.throws(()=>evidenceTypeForFeature("unsupported"),/unsupported_identity_feature_type/);
});
